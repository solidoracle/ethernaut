...
import "../src/levels/18_MagicNum/MagicNumFactory.sol";
...
contract MagicNumTest is DSTest {
    ...
    function testMagicNum() public {
        ...
        // 600c -- push 12 (runtime code start byte). This is because the bytecode for the initialization opcode will have 12 bytes in total. So the starting position for the runtime opcode will be index 12 or 0x0c
        ...
        // 6020 -- push number of bytes to return (length). Since we want to return an uint256, we need 32 bytes (not 1), so the hex of 32 is 20
        // 6080 -- push mem address to return (see 3 commands above)
        // f3   -- return
